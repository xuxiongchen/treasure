...
import com.cxdb.reflect.invoker.GetFieldInvoker;
import com.cxdb.reflect.invoker.Invoker;
import com.cxdb.reflect.invoker.MethodInvoker;
import com.cxdb.reflect.invoker.SetFieldInvoker;
import com.cxdb.reflect.property.PropertyNamer;
...
public class Reflector {
	...
	private Reflector(Class<?> clazz) {
		type = clazz;
		// 处理类的名称与别名
		cxdb_alias alias = clazz.getAnnotation(cxdb_alias.class);
		if (alias != null) {
	...
	private void addFields(Class<?> clazz) {
		...
		for (Field field : fields) {
			...
			// 处理cxdb_alias注解
			cxdb_alias alias = field.getAnnotation(cxdb_alias.class);
			if (alias != null) {
				alias_name.put(alias.value(), field.getName());
				name_alias.put(field.getName(), alias.value());
			}
			// 处理huisadb_ignore注解
			cxdb_ignore ignore = field.getAnnotation(cxdb_ignore.class);
			if (ignore != null) {
				field_ignore.put(field.getName(), true);
			}
		}
